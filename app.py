...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('/root/FFPbot/bot.log'),  # Логи в файл
...
def send_message(chat_id, text, reply_markup=None, parse_mode=None):
    ...
    try:
        response = requests.post(url, json=payload, timeout=10)  # Добавлен таймаут
...
def edit_message_caption(chat_id, message_id, caption, reply_markup=None, parse_mode=None):
    ...
    try:
        response = requests.post(url, json=payload, timeout=10)
        response.raise_for_status()
        logger.info(f"Сообщение обновлено для пользователя {chat_id}")
    except requests.exceptions.RequestException as e:
...
def check_subscription(user_id):
    ...
    try:
        ...
        logger.info(f"Проверка подписки для user_id {user_id}: {member_status}")
...
def main():
    ...
    retry_delay = 5  # Задержка между повторными попытками (секунд)
                    ...
                    if 'message' in update:
                        ...
                        if 'text' in message and message['text'] == '/start':
                            ...
                            if send_photo(chat_id, image_url, caption, reply_markup=get_subscription_keyboard()):
                                logger.info(f"Отправлено начальное сообщение пользователю {chat_id}")
                            else:
                                logger.warning(f"Не удалось отправить начальное сообщение пользователю {chat_id}")
                            ...
                            if check_subscription(user_id):
                                ...
                                if edit_message_caption(chat_id, message_id, caption):
                                    ...
                                    if send_message(chat_id, promo_text, reply_markup=get_ticket_keyboard(), parse_mode="HTML"):
                                        logger.info(f"Отправлен промокод пользователю {chat_id}")
                                    else:
                                        logger.warning(f"Не удалось отправить промокод пользователю {chat_id}")
                            else:
                                if edit_message_caption(chat_id, message_id, "К сожалению, ты всё ещё не подписан на наш канал.", reply_markup=get_subscription_keyboard()):
                                    logger.info(f"Обновлено сообщение для пользователя {chat_id} о необходимости подписки")
                                else:
                                    logger.warning(f"Не удалось обновить сообщение для пользователя {chat_id}")

                break  # Если запрос успешен, выходим из цикла повторных попыток

            except requests.exceptions.RequestException as e:
                logger.error(f"Ошибка при получении обновлений (попытка {retries + 1}/{max_retries}): {e}")
                retries += 1
                if retries < max_retries:
                    time.sleep(retry_delay)
                else:
                    logger.critical("Превышено максимальное количество повторных попыток. Перезапуск через 30 секунд...")
                    time.sleep(30)
