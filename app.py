        ...
        if 'message' in update:
            ...
            if callback_query['data'] == "check_subscription":
                if check_subscription(user_id):
                    edit_message_caption(chat_id, message_id, "Поздравляем! \nТы подписан на наши обновления, мы хотим отблагодарить тебя промокодом на наши мероприятия: JUNGLEISMASSIVE действует только при покупке билетов онлайн.", reply_markup=get_ticket_keyboard())
                else:
